...
import {FocusPokemon} from "./features/pages/Accueil/FocusPokemon";
import {PageListeTypePokemon} from "./features/pages/ListePokemon/PageListeTypePokemon";
import {PageListPokemon} from "./features/pages/ListePokemon/PageListPokemon";
import {Pokemon} from "./features/pages/DetailsPokemon/Pokemon";
import {FrontCard} from "./features/pages/DetailsPokemon/frontCard";
import {BackCard} from "./features/pages/DetailsPokemon/backCard";
import {Spinner} from "./features/pages/Spinner/spinner";
import {Mespokemon} from "./features/pages/Favorie/mesProkemon";
import { describe, expect, it} from "@jest/globals";
import  FavoriePokemonSlice, {AddPokemon} from "./features/redux/FavoriePokemonSlice";
...
describe('Page Acceuil', ()=>{
    ...
    test('Test aucun pokemon random est selectionner', () => {
        render(
            <Provider store={store}>
                <FocusPokemon/>
            </Provider>
        ...
        expect(screen.queryByText(/Pokedex/)).toBeNull();
    });

    test('Test Un pokemon Random est generer', async () => {
        render(
            <Provider store={store}>
                <FocusPokemon/>
            </Provider>
        );
        const message = await screen.findByText(/Pokedex/);
...
describe('Page Liste Pokemon ', ()=>{


    test('Test que aucun pokemon de la liste est charger', () => {
        render(
            <Provider store={store}>
                <PageListPokemon/>
            </Provider>
    ...
    test('Test la liste est charger', async () => {
        render(
            <Provider store={store}>
                <Router>
                    <PageListPokemon/>
        ...
        expect(screen.queryByText(/Détails/)).toBeNull();
        const message = await screen.findAllByText(/Détails/);
...
describe('Page Spinner', ()=>{
    test('Test que il y a bien une image dans le spinner', async () => {
        render(
            <Provider store={store}>
                <Spinner/>
            </Provider>
...
describe('Page details Pokemon', ()=>{


    test('Test qu\'il y a bien un spinner', async () => {
        render(
            <Provider store={store}>
                <Pokemon/>
            </Provider>
    ...
    test('Test que le front de la card affiche les info du pokemon', async () => {
         render(
            <Provider store={store}>
                <FrontCard pokemonInfo={pokemon} name={"pikachu"} description={"description"} version={"silver"}/>
            </Provider>
        );
        const nom = await screen.findByText(/pikachu/);
        await expect(nom).toBeInTheDocument();

        const description = await screen.findByText(/description/);
        await expect(description).toBeInTheDocument();

        const version = await screen.findByText(/silver/);
        await expect(version).toBeInTheDocument();
    });

    test('Test que le Back de la card contient les informations du pokemon', async () => {
        render(
            <Provider store={store}>
                <BackCard pokemonInfo={pokemon} pokemonSpecie={pokemon} name={"pikachu"} habitat={"Montagne"}/>
            </Provider>
        );
        const nom = await screen.findAllByText(/pikachu/);
        await expect(nom[0]).toBeInTheDocument();

        const habitat = await screen.findByText(/Montagne/);
        await expect(habitat).toBeInTheDocument();

        const attaque = await screen.findByText(/imposter/);
        await expect(attaque).toBeInTheDocument();
    });
});

describe('Page Favorie Pokemon', ()=>{

    it('Test page des favorie contient un pokemon nommer ditto', async () => {
        store.dispatch(
            AddPokemon({
        ...
        const name = await screen.findByText(/ditto/);
...
describe('Test Redux', ()=>{
    ...
    it('test FavoriePokemonSlice retourne la state initial', () => {
        expect(FavoriePokemonSlice(initialStateFavorie, {})).toMatchSnapshot()
    })

    it('test DetailsPokemonSlice retourne la state initial', () => {
        expect(DetailsPokemonSlice(initialStateDetails, {})).toMatchSnapshot()
    })


    it('test PaginationPokemonSlice retourne la state initial', () => {
        expect(PaginationPokemonSlice(initialStateListe, {})).toMatchSnapshot()
    })

    it('test AllPokemonSlice retourne la state initial', () => {
        expect(AllPokemonSlice(initialStateAll, {})).toMatchSnapshot()
    })
